...
def get_node_best_community(
  ...
  for other in nodes:
    if node != other and other in adjacency_matrix[node]: # only calculate modularity if not the same node
...
def aggregate_communities(
  graph: Graph,
  new_community_mapping: Dict[Node, Node]
) -> Graph:
