...
class Louvain(CommunityDetectionAlgorithm):
    ...
    def _get_node_best_community(
        ...
        for other in nodes:
            if (
                ...
                old_Q = self._calculate_modularity(
                    m=m,
                    nodes=nodes,
                ...
                new_Q = self._calculate_modularity(
                    m=m,
                    nodes=nodes,
                    adjacency_matrix=adjacency_matrix,
                    node_edge_weight_mapping=node_edge_weight_mapping,
    ...
    def _get_and_update_node_best_community(
        self,
        node: Node,
        m: Union[int, float],
        nodes: list[Node],
        ...
        best_node = self._get_node_best_community(
            node=node,
            m=m,
            nodes=nodes,
            adjacency_matrix=adjacency_matrix,
    ...
    def _optimise_modularity(self, graph: Graph) -> Dict[Node, Node]:
        ...
        for node in nodes:
            new_community_mapping = self._get_and_update_node_best_community(
                node=node,
                m=m,
                nodes=nodes,
                adjacency_matrix=adjacency_matrix,
    ...
    # --- Phase 2 --- #
    def _get_root_parent(self, node: Node, community_mapping: Dict[Node, Node]) -> Node:
        c = community_mapping[node]
        while c != community_mapping[node]:
            c = community_mapping[node]
        return c

    def _clean_community_mapping(self, community_mapping: Dict[Node, Node]) -> Dict[Node, Node]:
        ...
        for child in community_mapping.keys():
            parent = self._get_root_parent(child, community_mapping)
            new_mapping[child] = parent
        return new_mapping

    def _aggregate_communities(
        self, graph: Graph, new_community_mapping: Dict[Node, Node]
    ) -> Graph:
        cleaned_community_mapping = self._clean_community_mapping(new_community_mapping)
    ...
    def build_communities(self, graph: Graph, verbose=True) -> Graph:
        ...
        new_community_mapping = self._optimise_modularity(graph)
        ...
        while old_community_mapping != new_community_mapping:
            ...
            if new_graph is None:
                new_graph = self._aggregate_communities(graph, old_community_mapping)
            else:
                new_graph = self._aggregate_communities(new_graph, old_community_mapping)
            new_community_mapping = self._optimise_modularity(new_graph)
